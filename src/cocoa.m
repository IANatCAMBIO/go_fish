...
//     Cascade All, About…, Settings…, Quit.
...
#include "cocoa.h"
#include "maui_png.h"  // generated by the Makefile: maui_png[], maui_png_len
#include <dlfcn.h>
#include <limits.h>
...
// =========================================================================
//
// Reached from the status menu. Everything it shows is baked in at build
// time: maui.png is embedded as a byte array (same xxd treatment as the
// menu-bar hook, so a loose binary run outside the .app still has it), the
// version comes from the bundle's Info.plist, and the compile date is the
// compiler's __DATE__ for this translation unit — cocoa.m is rebuilt
// whenever any source changes, so it tracks the binary.

// maui.png is drawn at this fraction of its natural pixel size, with its
// corners rounded by this radius.
static const CGFloat kAboutImageScale  = 0.85;
static const CGFloat kAboutImageRadius = 12;
static NSString *const kAboutLinkURL    = @"http://camb.io";
static NSString *const kAboutLicenseURL =
    @"https://www.gnu.org/licenses/gpl-3.0.html#license-text";

@interface GFAboutWindowController : NSObject
@property (nonatomic, strong) NSWindow *aboutWindow;
- (void)showAbout:(id)sender;
@end
static GFAboutWindowController *gAboutController = nil;

@implementation GFAboutWindowController

// Short version string from Info.plist, so the plist stays the single source
// of truth. A loose binary has no bundle to read, hence the "dev" fallback.
- (NSString *)versionString {
    NSString *v = [[NSBundle mainBundle]
        objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
    return v.length ? v : @"dev";
}

// Secondary-colour caption used for the version and compile-date rows.
- (NSTextField *)infoLabel:(NSString *)text {
...
// Caption in which the `linkText` substring is drawn as a hyperlink to `url`.
// Pass the whole string as linkText for a bare-URL row.
//
// An NSTextView rather than an NSTextField: a text field only routes
// NSLinkAttributeName clicks through its field editor, which does not exist
// until the field is first responder, so the first click on the link is
// swallowed making it one. A text view opens the URL on the first click and
// gives the pointing-hand cursor for free.
- (NSTextView *)linkLabel:(NSString *)text
                 linkText:(NSString *)linkText
                      url:(NSString *)url {
    NSFont *font = [NSFont systemFontOfSize:[NSFont smallSystemFontSize]];
    // Centring lives in the attributed string; a text view has no alignment
    // property of its own to set from the layout pass.
    ...
    NSRange r = [text rangeOfString:linkText];
    ...
    tv.editable        = NO;
    tv.selectable      = YES;   // link tracking is part of selection handling
    tv.textContainerInset = NSZeroSize;
    tv.textContainer.lineFragmentPadding = 0;
    // Styling for the link range. Set here rather than in the attributed
    // string because a text view paints link ranges from this dictionary and
    // would otherwise override the colours with its own defaults.
...
- (void)buildWindow {
    ...
    NSImage *maui = [[NSImage alloc] initWithData:
        [NSData dataWithBytes:maui_png length:maui_png_len]];
    // NSImage.size is in points, which for maui.png's 144-dpi tag is half its
    // 210x206 pixel size. Scaling from points is what keeps it pixel-crisp on
    // a Retina display: the 2x backing store asks for exactly the pixels the
    // file has, where scaling from the pixel count would upsample it.
    NSSize natural = maui ? maui.size : NSZeroSize;
    CGFloat imgW = natural.width  * kAboutImageScale;
    CGFloat imgH = natural.height * kAboutImageScale;

    // Rows are built before the window because its width is driven by the
    // widest of them — measuring beats guessing a constant that a larger
    // system font size would silently clip.
    NSTextField *name = [NSTextField labelWithString:@"Go Fish"];
    name.font = [NSFont boldSystemFontOfSize:22];
    name.alignment = NSTextAlignmentCenter;

    NSTextField *ver = [self infoLabel:
        [NSString stringWithFormat:@"Version: %@", [self versionString]]];

    // __DATE__ space-pads a single-digit day ("Sep  8 2026"); collapse the
    // padding so the label reads as a normal date.
    NSString *built = [@(__DATE__) stringByReplacingOccurrencesOfString:@"  "
                                                             withString:@" "];
    NSTextField *date = [self infoLabel:
        [NSString stringWithFormat:@"Compiled %@", built]];

    NSTextView *lic = [self linkLabel:@"Released under the GPL 3.0 licence."
                             linkText:@"GPL 3.0 licence"
                                  url:kAboutLicenseURL];

    NSTextView *link = [self linkLabel:kAboutLinkURL
                              linkText:kAboutLinkURL
                                   url:kAboutLinkURL];

    CGFloat widest = imgW;
    for (NSTextField *f in @[name, ver, date]) {
        widest = MAX(widest, ceil(f.fittingSize.width));
    }
    for (NSTextView *t in @[lic, link]) {
        widest = MAX(widest, ceil(t.textStorage.size.width));
    }
    const CGFloat W = MAX(300, widest + 2 * margin);

    // Laid out bottom-up: each row records its origin, then advances y past
    // its own height plus the gap below the row above it. The top gap is
    // tighter than the side/bottom margin because the title's 28pt row is
    // already taller than its 22pt glyphs, which adds its own slack above.
    const CGFloat topGap = 8;

    CGFloat y = margin;
    CGFloat linkY = y; y += 18 + 14;
    CGFloat licY  = y; y += 18 + 10;
    CGFloat dateY = y; y += 16 + 2;
    CGFloat verY  = y; y += 16 + 12;
    CGFloat imgY  = y; y += imgH + (imgH > 0 ? 12 : 0);
    CGFloat nameY = y; y += 28 + topGap;
    ...
    win.title = @"About";
    [win center];
    // The controller holds the only strong reference; closing the window must
    // not deallocate it, or reopening from the menu would crash.
    ...
    if (maui) {
        NSImageView *iv = [[NSImageView alloc] initWithFrame:
            NSMakeRect((W - imgW) / 2, imgY, imgW, imgH)];
        iv.image = maui;
        iv.imageScaling = NSImageScaleProportionallyUpOrDown;
        // The frame matches the image's aspect ratio exactly, so clipping the
        // layer to a rounded rect rounds the artwork rather than empty
        // letterbox padding around it.
        iv.wantsLayer = YES;
        iv.layer.cornerRadius  = kAboutImageRadius;
        iv.layer.masksToBounds = YES;
        [cv addSubview:iv];
    }

    void (^place)(NSView *, CGFloat, CGFloat) =
        ^(NSView *v, CGFloat ly, CGFloat h) {
            v.frame = NSMakeRect(margin, ly, W - 2 * margin, h);
            [cv addSubview:v];
        };
...
- (void)showAbout:(id)sender {
    if (!self.aboutWindow) [self buildWindow];
    [self.aboutWindow makeKeyAndOrderFront:nil];
    // Accessory apps aren't activated by opening a window, so without this
    // the About window appears behind whatever the user was just using.
...
- (void)showAbout:(id)sender {
    [gAboutController showAbout:nil];
}
- (void)showSettings:(id)sender {
...
static void installStatusItem(const void *iconBytes, int iconLen) {
    ...
    gAboutController = [GFAboutWindowController new];
    NSMenuItem *aboutItem = [[NSMenuItem alloc] initWithTitle:@"About…"
                                                       action:@selector(showAbout:)
                                                keyEquivalent:@""];
    aboutItem.target = gStatusHandler;
